...
- `health` is read-only. `health` and `reconcile` report `ok`,
  `repair_needed`, `manual_review_needed`, or `blocked` and recommend the next
  safe operator action. `repair_needed` is reserved for a valid stale pending
  worker result that can be applied with explicit recovery.
...
- `health` и `reconcile` — read-only команды. Они показывают `ok`,
  `repair_needed`, `manual_review_needed` или `blocked` и рекомендуют следующий
  безопасный шаг оператора. `repair_needed` используется только для валидного
  stale pending worker result, который можно применить явным recovery.
