...
- `prepare-runtime --package` is a controlled capability. Package decisions
  ...
  content.
- `health` is read-only. It reports `ok`, `repair_needed`, or
  `manual_review_needed` and recommends the next safe operator action.
...
- `prepare-runtime --package` — controlled capability. Решение об установке
  ...
  retrieved content.
- `health` — read-only команда. Она показывает `ok`, `repair_needed` или
  `manual_review_needed` и рекомендует следующий безопасный шаг оператора.
